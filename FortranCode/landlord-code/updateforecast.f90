...
!        Xt(t-99,ngpAgg*ngpAgg+ngpAgg+iAgg)=log(Prpath(t))-log(Phpath(t))
...
!       aa2p(iAgg,:)=beta_ols(ngpAgg*ngpAgg+ngpAgg+iAgg)
       do iAggp=1,ngpAgg
!            aa1p(iAgg,iAggp)=beta_ols(ngpAgg*ngpAgg+(iAgg-1)*ngpAgg+iAggp)
...
!        Xt(t-99,ngpAgg*ngpAgg+iAgg)=log(Phpath(t))
!        Xt(t-99,ngpAgg*ngpAgg+ngpAgg+iAgg)=log(Prpath(t))-log(Phpath(t))
        Xt(t-99,ngpAgg*ngpAgg+iAgg)=log(Prpath(t))-log(Phpath(t))
        yyt(t-99)=log(Prpath(t+1))-log(Phpath(t+1))
    ...
    do iAgg=1,ngpAgg
!       bb1p(iAgg,:)=beta_ols(ngpAgg*ngpAgg+iAgg)
!       bb2p(iAgg,:)=beta_ols(ngpAgg*ngpAgg+ngpAgg+iAgg)
       bb2p(iAgg,:)=beta_ols(ngpAgg*ngpAgg+iAgg)

       do iAggp=1,ngpAgg
          bb0p(iAgg,iAggp)=beta_ols((iAgg-1)*ngpAgg+iAggp)
