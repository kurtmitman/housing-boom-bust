...
!    DO gij = 1,Jwork
!       print*,gij,sum(zygrid(gij,:)*ygrid(gij,1,:,1,1)),sum(zygrid(gij,:)*ypsgrid(gij,1,:,1))
!    ENDDO
!    stop
    ...
    uChi_h = (1.0d0/(1.0d0+(Pr**(alph/(1.0d0-alph)))*((nunu/(1.0d0-nunu))**(1.0d0/(1.0d0-alph)))))/Pr
        ...
        do bc = 1,nb
            ...
            do hcc=1,nhr
                ...
                bWRJ(index)=btemp
                consRJ(index)=uChi_c*(gliq-FnQb(btemp,0.0d0)*btemp)
                hWRJ(index)=uChi_h*(gliq-FnQb(btemp,0.0d0)*btemp)
            ...
            do hc=1,nh
                ...
                do bc=1,nb
                    ...
                    do hcc=1,nhr
                        ...
                        index=hcc
                        gliq=Trans(giPh)+bjp+y-FnTaxM(y,GridH(hc)*(GridM(mc)+GridL(lc)),min(0.0d0,bj))
                        bccmax=(gliq-cmin)*(1.0d0+rf)
                        bccmin=0.0d0
                        bccmid=(bccmax+bccmin)/2.0d0
                    ...
                    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    !pm=GridH(hc)*GridM(mc)*(1.0d0+rm)
                    pm=FnPm(GridH(hc)*GridM(mc),gij+Jwork)
                    if(mc .eq. 1) pm=0.0d0
                    mp=(GridM(mc)*GridH(hc))*(1.0d0+rm)-pm

                    gliq=Trans(giPh)+bj+y-FnTaxM(y,GridH(hc)*(GridM(mc)+GridL(lc)),min(0.0d0,bj))-pm-GridH(hc)*GridL(lc)*(1.0d0+rl)-FnMaint(GridH(hc))+(1.0d0-rent_tax)*Pr*GridH(hc)
                    gih = hc
                    ghouse = GridH(gih)
                    gmort = mp
                    bccmax=(gliq-cmin)*(1.0d0+rf)
                    ...
                    !bccmin=min(0.0d0,-HELOCGrid(giAgg)*gPh*ghouse)
                    bccmid=(bccmax+bccmin)/2.0d0
                    ...
                    consPPolRet(bc,lc,mc,hc,giExo,giPh,gij)=(gliq-FnQb(btemp,gPh*ghouse-mp)*btemp)*uChi_c

                    bPPolRet(bc,lc,mc,hc,giExo,giPh,gij)=btemp
                    hconsPPolRet(bc,lc,mc,hc,giExo,giPh,gij)=(gliq-FnQb(btemp,gPh*ghouse-mp)*btemp)*uChi_h
...
!                    gliq=bj+y-FnTax(y)-(1.0d0-0.5d0/(dble(BiAnnual)+1.0d0))*Pr*GridHR(1)
                    ...
                    consFPolRet(bc,lc,mc,hc,giExo,giPh,gij)=uChi_c*(gliq-FnQb(btemp,0.0d0)*btemp)
                    hconsFPolRet(bc,lc,mc,hc,giExo,giPh,gij)=uChi_h*(gliq-FnQb(btemp,0.0d0)*btemp) !GridH(hc)*0.5d0/(dble(BiAnnual)+1.0d0)+(1.0d0-0.5d0/(dble(BiAnnual)+1.0d0))*GridHR(1)
...
!                    print*,bc,lc,mc,hc,giExo,giPh
!                    print*,WforeRet(bc,lc,mc,hc,giExo,giPh,gij) ,WsellRet(bc,lc,mc,hc,giExo,giPh,gij),WpayRet(bc,lc,mc,hc,giExo,giPh,gij)
...
!                        if( (GridM(mc) .GT. LTVGrid(giAgg)*gPh) .OR. (FnPm(GridM(mc)*GridH(hc),gij+1+Jwork) .GT. LTIGrid(giAgg)*y) .OR. ( (DoInterestCap(giAgg) .eq. 1) .and. (qmret_e(bc,lc,mc,hc,giExo,giPh,gij+1) .lt. InterestCap)) ) then



!                        if( ( (GridM(mc) .GT. LTVGrid(giAgg)*gPh) .AND. (DoMqm .eq. 0)) .OR. ( (qmret_e(bc,lc,mc,hc,giExo,giPh,gij+1)*GridM(mc) .GT. LTVGrid(giAgg)*gPh) .AND. (DoMqm .eq. 1))  .OR. (FnPm(GridM(mc)*GridH(hc),gij+1+Jwork) .GT. LTIGrid(giAgg)*y) .OR. ( (DoInterestCap(giAgg) .eq. 1) .and. (qmret_e(bc,lc,mc,hc,giExo,giPh,gij+1) .lt. InterestCap)) ) then
...
!                            qmret(bc,lc,mc,hc,giExo,giPh,gij+1)=1.0d0
                        else
...
!                       if( ((GridM(mc)+GridL(lc)) .GT. HELOCGrid(giAgg)*gPh) .OR. ((GridM(mc)+GridL(lc)) .GT. LTIGrid(giAgg)*y) ) qlret(bc,lc,mc,hc,giExo,giPh,gij+1) = 0.0d0
...
!            if(Display==2)            print*,'Emax ok'
            do bc = 1,nb
                ...
                do hcc=1,nhr
                    ...
                    bR(index)=btemp
                    consR(index)=uChi_c*(gliq-FnQb(btemp,0.0d0)*btemp)
                    hconsR(index)=uChi_h*(gliq-FnQb(btemp,0.0d0)*btemp)
                ...
                do hcc=1,nh
                    ...
                    do mcc=1,nm
                        do lcc=1,nl
                            ...
                            gmort=GridM(gim)
                            gloc=GridL(gil)
                            gliq=Trans(giPh)+bj+y-FnTax(y)-Ph*GridH(hcc)-fc+(1.0d0-rent_tax)*Pr*GridH(hcc) !-FnMaint(GridH(hcc))
...
!                            bccmin=min(-HELOCGrid(giAgg)*((gPh-gmort)*ghouse),0.0d0)
                ...
                consBPolRet(bc,giExo,giPh,gij)   =   uChi_c*consB(polind)
                hBPolRet(bc,giExo,giPh,gij)      =   GridH(hB(polind))
                hIBPolRet(bc,giExo,giPh,gij)     =   hB(polind)
                hconsBPolRet(bc,giExo,giPh,gij)  =   uChi_h*consB(polind)
                    ...
                    do hcc=1,nh
                        ...
                        do mcc=1,nm
                            do lcc=1,nl
                                ...
                                gmort=GridM(gim)
                                gloc=GridL(gil)
                                gliq=Trans(giPh)+bj+y-FnTax(y)-Ph*GridH(hcc)-fc-FnMaint(GridH(hcc))+(1.0d0-rent_tax)*Pr*GridH(hcc)
...
!                                bccmin=0.0d0
!                                bccmin=min(0.0d0,-HELOCGrid(giAgg)*(gPh-gmort)*ghouse)
                                bccmid=(bccmax+bccmin)/2.0d0
                                ftemp1=FnVFBRet(0.0d0)
                                ftemp2=golden(bccmin,bccmid,bccmax,FnVFBRet,vftol,btemp)
                    ...
                    consBPolRetS(bc,giExo,giPh,gij)   =   uChi_c*consB(polind)
                    hBPolRetS(bc,giExo,giPh,gij)      =   GridH(hB(polind))
                    hIBPolRetS(bc,giExo,giPh,gij)     =   hB(polind)
                    hconsBPolRetS(bc,giExo,giPh,gij)  =   uChi_h*consB(polind)
...
!             if(Display==2)            print*,'Renter ok'


             !!!! Now Solve Homeowner's problem
             !call spline(GridB,WbuyRet(:,giExo,giPh,gij),coef1,coef2,coef3,nb)
...
!                        gliq=bj+y-FnTax(y)-(1.0d0-0.5d0/(dble(BiAnnual)+1.0d0))*Pr*GridHR(1)
                        ...
                        fc=FCGrid(giAgg)
                            do mcc=max(mc,2),nm
                               ...
                               do lcc=1,nl
                                    ...
                                    gmort=GridM(gim)
                                    gloc=GridL(gil)
                                    gliq=Trans(giPh)+bj+y-FnTaxM(y,GridH(hc)*(GridM(mc)+GridL(lc)),min(0.0d0,bj))-(GridM(mc)*(1.0d0+rm)+GridL(lc)*(1.0d0+rl))*GridH(hc)-fc-FnMaint(GridH(hc))+(1.0d0-rent_tax)*Pr*GridH(hc)
                            ...
                            consNPolRet(bc,lc,mc,hc,giExo,giPh,gij)=uChi_c*consN(polind)
                            hconsNPolRet(bc,lc,mc,hc,giExo,giPh,gij)=uChi_h*consN(polind)
                        ...
                        do lcc=1,nl
                                ...
                                gmort=mp
                                gloc=GridL(gil)
                                gliq=Trans(giPh)+bj+y-FnTaxM(y,GridH(hc)*(GridM(mc)+GridL(lc)),min(0.0d0,bj))-pm-GridL(lc)*ghouse*(1.d0+rl)-FnMaint(GridH(hc))+(1.0d0-rent_tax)*Pr*GridH(hc)
...
!                                bccmin=min(-HELOCGrid(giAgg)*(gPh*ghouse-gmort),0.0d0)
                        ...
                        do while(((GridM(mc)*GridH(hc))*(1.0d0+rm)-pm .GT. (GridM(mcc)*GridH(hc)) .AND. mcc.LT.mc)) ! .OR. (mcc .eq. 1))
                            do lcc=1,nl
                                    ...
                                    gloc=GridL(gil)
                                    gmort=GridM(gim)
                                    gliq=Trans(giPh)+bj+y-FnTaxM(y,GridH(hc)*(GridM(mc)+GridL(lc)),min(0.0d0,bj))-GridL(lc)*ghouse*(1.d0+rl)+((GridM(mcc)*GridH(hc))-(GridM(mc)*GridH(hc))*(1.0d0+rm))-FnMaint(GridH(hc))+(1.0d0-rent_tax)*Pr*GridH(hc)
...
!                                    bccmin=min(-HELOCGrid(giAgg)*((gPh-gmort)*ghouse),0.0d0)
                        ...
                        consPPolRet(bc,lc,mc,hc,giExo,giPh,gij)=uChi_c*consP(polind)
                        hconsPPolRet(bc,lc,mc,hc,giExo,giPh,gij)=uChi_h*consP(polind)
...
!                        if(mc.eq.1 .and. gij .lt. 5 .and. giPh .eq. ngpPh/2+1) then
!                           print*,bc,hc,giPh
!                           print*,helpVals
!                           print*,polind
!                           pause
...
!                                pm=FnPm(GridM(mc)*GridH(hc),gij+Jwork)
!                                mp=(GridM(mc)*GridH(hc))*(1.0d0+rm)-pm
