...
!lsim(giPh,in)=0.0d0
bsim=0.0d0
!hconssim(giPh,in)=GridH(hconsB(polind))
!csim(giPh,in)=consB(polind)
!refisim(giPh,in)=0
!foresim(giPh,in)=0
!sellsim(giPh,in)=0
!hsimI(giPh,in)=1
    ...
    do in=1,nsim
        ...
        select case(ij)
            ...
            case(1:Jwork)
                ...
                !print*,it,'act sim'
                !print*,ij,simtrans(it)%ownsim(in),simtrans(it)%msim(in),simtrans(it)%bsim(in)
...
!                                    ftemp2=FnVFB(GridB(2))
                        ...
                        simtrans(it)%hconssim(in)=GridH(hB(polind)) !simtrans(it+1)%hsim(in)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    ...
                    !print*,Wbuysim,Wrentsim,helpWfore,simtrans(it)%thetasim(in)
                    ...
                    if(simtrans(it)%thetasim(in) .eq. 1) then !Hit by moving shock
                        ...
                        select case(polind)
                            ...
                            case(4) !Payment optimal
                                ...
                                !print*,mPpol(1,ljI,:,hjI,giExo,4,gij),gPh,bj,simtrans(it)%msim(in)
                                !pause
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            case(Jwork+1:Jtot-1)
                ...
                giR=ip !+ngpR*nint(real(in)/real(nsim))*(ngpPv-1)/ngpPv
                iR=giR
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    ...
                    !print*,Wbuysim,Wrentsim,Wpaysim,bjp,bjps
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      !$OMP END PARALLEL DO
      print*,sum(simtrans(it)%hconssim(:))/real(nsim),Ht
