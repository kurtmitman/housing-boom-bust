...
double precision                    :: bjp, bj, Wsellbuy,Wsellrent,bjps,nunu,cexp
...
double precision                    :: uChi_c,uChi_h !CES shares
...
!    DO gij = 1,Jwork
!       print*,gij,sum(zydist(gij,:)*ygrid(gij,1,:,1,1)),sum(zydist(gij,:)*ypsgrid(gij,1,:,1))
!       !print*,zydist(gij,:)
!    ENDDO
!    stop
...
!rf+mortmarkup
            ...
            uChi_h = (1.0d0/(1.0d0+(Pr**(alph/(1.0d0-alph)))*((nunu/(1.0d0-nunu))**(1.0d0/(1.0d0-alph)))))/Pr
            !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        ...
                        do bc=1,nb
                            ...
                            do giAggp = 1,ngpAgg
                                do giWp = 1, ngpW
                                    ...
                                    EVown(bc,lc,mc,hc,gij)=EVown(bc,lc,mc,hc,gij)+zytrans(gij,giW,giWp)*transMatAgg(giAgg,giAggp)*(PtoV1(giPh,giAgg,giAggp)*Vown(bc,lc,mc,hc,giExop,PtoP1(giPh,giAgg,giAggp),gij+1)+PtoV2(giPh,giAgg,giAggp)*Vown(bc,lc,mc,hc,giExop,PtoP2(giPh,giAgg,giAggp),gij+1) + PrtoV1(giPh,giAgg,giAggp)*Vown(bc,lc,mc,hc,giExop,PrtoP1(giPh,giAgg,giAggp),gij+1)+PrtoV2(giPh,giAgg,giAggp)*Vown(bc,lc,mc,hc,giExop,PrtoP2(giPh,giAgg,giAggp),gij+1))


                                   qm_e(bc,lc,mc,hc,giExo,giPh,gij+1)=qm_e(bc,lc,mc,hc,giExo,giPh,gij+1) + zytrans(gij,giW,giWp)*transMatAgg(giAgg,giAggp)*((1.0d0-thet(gij))*(PtoV1(giPh,giAgg,giAggp)*bankgain_mort(bc,lc,mc,hc,giExop,PtoP1(giPh,giAgg,giAggp))+PtoV2(giPh,giAgg,giAggp)*bankgain_mort(bc,lc,mc,hc,giExop,PtoP2(giPh,giAgg,giAggp)) + PrtoV1(giPh,giAgg,giAggp)*bankgain_mort(bc,lc,mc,hc,giExop,PrtoP1(giPh,giAgg,giAggp))+PrtoV2(giPh,giAgg,giAggp)*bankgain_mort(bc,lc,mc,hc,giExop,PrtoP2(giPh,giAgg,giAggp)))+thet(gij)*(PtoV1(giPh,giAgg,giAggp)*bankgain_mort_move(bc,lc,mc,hc,giExop,PtoP1(giPh,giAgg,giAggp))+PtoV2(giPh,giAgg,giAggp)*bankgain_mort_move(bc,lc,mc,hc,giExop,PtoP2(giPh,giAgg,giAggp)) +PrtoV1(giPh,giAgg,giAggp)*bankgain_mort_move(bc,lc,mc,hc,giExop,PrtoP1(giPh,giAgg,giAggp))+PrtoV2(giPh,giAgg,giAggp)*bankgain_mort_move(bc,lc,mc,hc,giExop,PrtoP2(giPh,giAgg,giAggp))))
...
!                              elseif( (GSE .eq. 1) .AND. (GridM(mc) .LE. GridC(giC)*gPh)) then
!                                 qm(bc,lc,mc,hc,giExo,giPh,gij+1)=1.0d0
                              else
...
!                              if((GSE .eq. 1) .AND. (giW .lt. ngpzy/2+1) .and. (giC .lt. ngpC) .and. (gij .LT. 6))  qm(bc,lc,mc,hc,giExo,giPh,gij+1)=(1.04*qm(bc,lc,mc,hc,giExo,giPh,gij+1)
                ...
                do bc=1,nb
                    ...
                    do giAggp = 1,ngpAgg
                        do giWp = 1, ngpW
                            giExop = (giAggp-1)*ngpW+giWp
                            EVrent(bc,gij)=EVrent(bc,gij)+zytrans(gij,zWind(giW),zWind(giWp))*transMatAgg(giAgg,giAggp)*(PtoV1(giPh,giAgg,giAggp)*Vrent(bc,giExop,PtoP1(giPh,giAgg,giAggp),gij+1)+PtoV2(giPh,giAgg,giAggp)*Vrent(bc,giExop,PtoP2(giPh,giAgg,giAggp),gij+1) +PrtoV1(giPh,giAgg,giAggp)*Vrent(bc,giExop,PrtoP1(giPh,giAgg,giAggp),gij+1)+PrtoV2(giPh,giAgg,giAggp)*Vrent(bc,giExop,PrtoP2(giPh,giAgg,giAggp),gij+1))
                        enddo
                    enddo
                enddo
            else
                    ...
                    do mc=1,nm
                        ...
                        do bc=1,nb
                            ...
                            do giAggp = 1,ngpAgg
                                ...
                                EVown(bc,lc,mc,hc,gij) = EVown(bc,lc,mc,hc,gij)+transMatAgg(giAgg,giAggp)*(PtoV1(giPh,giAgg,giAggp)*VownRet(bc,lc,mc,hc,giExop,PtoP1(giPh,giAgg,giAggp),1)+PtoV2(giPh,giAgg,giAggp)*VownRet(bc,lc,mc,hc,giExop,PtoP2(giPh,giAgg,giAggp),1)+PrtoV1(giPh,giAgg,giAggp)*VownRet(bc,lc,mc,hc,giExop,PrtoP1(giPh,giAgg,giAggp),1)+PrtoV2(giPh,giAgg,giAggp)*VownRet(bc,lc,mc,hc,giExop,PrtoP2(giPh,giAgg,giAggp),1))
                                ...
                                qmret_e(bc,lc,mc,hc,giExo,giPh,1)=qmret_e(bc,lc,mc,hc,giExo,giPh,1)+transMatAgg(giAgg,giAggp)*(PtoV1(giPh,giAgg,giAggp)*bankgain_mort(bc,lc,mc,hc,giExop,PtoP1(giPh,giAgg,giAggp))+PtoV2(giPh,giAgg,giAggp)*bankgain_mort(bc,lc,mc,hc,giExop,PtoP2(giPh,giAgg,giAggp))+PrtoV1(giPh,giAgg,giAggp)*bankgain_mort(bc,lc,mc,hc,giExop,PrtoP1(giPh,giAgg,giAggp))+PrtoV2(giPh,giAgg,giAggp)*bankgain_mort(bc,lc,mc,hc,giExop,PrtoP2(giPh,giAgg,giAggp)))
...
!                             if((GridM(mc) .GT. LTVGrid(giAgg)*gPh) .OR. (GridM(mc)*GridH(hc) .GT. LTIGrid(giAgg)*y)) then
!                             if( (GridM(mc) .GT. LTVGrid(giAgg)*gPh) .OR. (FnPm(GridM(mc)*GridH(hc),gij+1) .GT. LTIGrid(giAgg)*y) .OR.( (DoInterestCap(giAgg) .eq. 1) .and. (qmret_e(bc,lc,mc,hc,giExo,giPh,1) .lt. InterestCap))) then
...
!                            elseif( (GSE .eq. 1) .AND. (GridM(mc) .LE. GSELim*gPh) .AND. (GridM(mc)*GridH(hc) .LT. GridC(giC)*y)) then
!                                qmret(bc,lc,mc,hc,giExo,giPh,1)=1.0d0
                            else
                                ! qmret(bc,lc,mc,hc,giExo,giPh,1)=qmret_e(bc,lc,mc,hc,giExo,giPh,1)
                                if(OnlyBankBelief .eq. 0) then
                ...
                do bc=1,nb
                    ...
                    do giAggp = 1,ngpAgg
                        giExop = (giAggp-1)*ngpR+giR
                        EVrent(bc,gij)=EVrent(bc,gij)+transMatAgg(giAgg,giAggp)*(PtoV1(giPh,giAgg,giAggp)*VrentRet(bc,giExop,PtoP1(giPh,giAgg,giAggp),1)+PtoV2(giPh,giAgg,giAggp)*VrentRet(bc,giExop,PtoP2(giPh,giAgg,giAggp),1)+PrtoV1(giPh,giAgg,giAggp)*VrentRet(bc,giExop,PrtoP1(giPh,giAgg,giAggp),1)+PrtoV2(giPh,giAgg,giAggp)*VrentRet(bc,giExop,PrtoP2(giPh,giAgg,giAggp),1))
                    enddo
                enddo
...
!                    do bcc=1,nb
!                        index=(hcc-1)*nb+bcc
!                        consR(index)=bj+y-FnTax(y)-FnQb(GridB(bcc),0.0d0)*GridB(bcc)-Pr*GridH(hcc)
!                        hconsR(index)=hcc
!                        bR(index)=bcc
!                        helpWR(index)=urent(max(cmin,consR(index)),GridH(hcc),gij)+bet*EVrent(bcc,giExo,giPh,gij)
!                    enddo
!                enddo
                ...
                consRpol(bc,giExo,giPh,gij)=uChi_c*consR(polind)
                hconsRpol(bc,giExo,giPh,gij)=uChi_h*consR(polind)
...
!                    print*,'Negative consumption Renter'
!                    print*,bc,giW,giAgg
!                    print*,gij
!                    pause
                endif
                ...
                do hcc=1,nh
                    ...
                    do mcc=1,nm
                        do lcc=1,nl
                            ...
                            gmort=GridM(gim)
                            gloc=GridL(gil)
                            gliq=bj+y-FnTax(y)-Ph*GridH(hcc)-fc+Trans(giPh)+(1.0d0-rent_tax)*Pr*GridH(hcc) !Now can trade rental services immediately
                            ftemp1=FnVFB(bmin)
            ...
            do bc=1,nbs
                ...
                if(bj+y-FnTax(y)-Pr*GridHR(1) .gt. cmin) then
                    ...
                    consRpolS(bc,giExo,giPh,gij)=uChi_c*consR(polind)
                    hconsRpolS(bc,giExo,giPh,gij)=uChi_h*consR(polind)
                    ...
                    do hcc=1,nh
                        ...
                        do mcc=1,nm
                            do lcc=1,nl
                                ...
                                gmort=GridM(gim)
                                gloc=GridL(gil)
                                gliq=bj+y-FnTax(y)-Ph*GridH(hcc)-fc-FnMaint(GridH(hcc))+Trans(giPh)+(1.0d0-rent_tax)*Pr*GridH(hcc)
...
!                                bccmin=0.0d0
                                ...
                                helpWB(index)=-ftemp1
    !                            print*,bj+y,btemp,consB(index)
    !                            print*,ql(inds(1),gil,gim,gih,giExo,giPh,gij+1)*gloc,ql(inds(2),gil,gim,gih,giExo,giPh,gij+1)*gloc
...
!                        gliq=bj+y-FnTax(y)-(1.0d0-0.5d0/(dble(BiAnnual)+1.0d0))*Pr*GridHR(1)
                        ...
                        consFpol(bc,lc,mc,hc,giExo,giPh,gij)=uChi_c*(gliq-FnQb(btemp,0.0d0)*btemp)
                        hconsFpol(bc,lc,mc,hc,giExo,giPh,gij)=uChi_h*(gliq-FnQb(btemp,0.0d0)*btemp) !GridH(hc)*0.5d0/(dble(BiAnnual)+1.0d0)+(1.0d0-0.5d0/(dble(BiAnnual)+1.0d0))*GridHR(1)
...
!                        if(mc .gt. 1) then
                        ...
                        do mcc=max(mc,2),nm
                            ...
                            do lcc=1,nl
                                ...
                                gmort=GridM(gim)
                                gloc=GridL(gil)
                                gliq=bj+y-FnTaxM(y,GridH(hc)*(GridM(mc)+GridL(lc)),min(0.0d0,bj))-(GridM(mc)*(1.0d0+rm)+GridL(lc)*(1.0d0+rl))*GridH(hc)-fc-FnMaint(GridH(hc))+Trans(giPh)+(1.0d0-rent_tax)*Pr*GridH(hc)
                            ...
                            consNpol(bc,lc,mc,hc,giExo,giPh,gij)=uChi_c*consN(polind)
                            hconsNpol(bc,lc,mc,hc,giExo,giPh,gij)=uChi_h*consN(polind)
                        ...
                        do lcc=1,nl
                            ...
                            gmort=mp
                            gloc=GridL(gil)
                            gliq=bj+y-FnTaxM(y,GridH(hc)*(GridM(mc)+GridL(lc)),min(0.0d0,bj))-pm-GridL(lc)*ghouse*(1.0d0+rl)-FnMaint(GridH(hc))+Trans(giPh)+(1.0d0-rent_tax)*Pr*GridH(hc)
...
!                            bccmin=min(-HELOCGrid(giAgg)*(gPh*ghouse-gmort),0.0d0)
                        ...
                        do while(((GridM(mc)*GridH(hc))*(1.0d0+rm)-pm .GT. (GridM(mcc)*GridH(hc)) .AND. mcc .LT. mc))! .OR. (mcc .EQ. 1))
                            do lcc=1,nl
                                ...
                                gmort=GridM(mcc)
                                gloc=GridL(gil)
                                gliq=bj+y-FnTaxM(y,GridH(hc)*(GridM(mc)+GridL(lc)),min(0.0d0,bj))-GridL(lc)*ghouse*(1.d0+rl)+((GridM(mcc)*GridH(hc))-(GridM(mc)*GridH(hc))*(1.0d0+rm))-FnMaint(GridH(hc))+Trans(giPh)+(1.0d0-rent_tax)*Pr*GridH(hc)
                                bccmax=gliq*(1.0d0+rf)
                                if(HELOCVal .eq. 1) then
                                   bccmin=-HELOCGrid(giAgg)*gPh*ghouse
                                else
...
!                                bccmin=min(-HELOCGrid(giAgg)*(gPh-gmort)*ghouse,0.0d0)
                        ...
                        consPpol(bc,lc,mc,hc,giExo,giPh,gij)=uChi_c*consP(polind)
                        hconsPpol(bc,lc,mc,hc,giExo,giPh,gij)=uChi_h*consP(polind)
