...
MarkovSwitch=0
!HELOCVal=0
!!!!! Make grid for house price shocks
...
GridC(ngpC)=0.8d0
GridC(1)=LTVMax
PhgridTemp(ngpPh)  = 0.5d0 !(real(ngpPh)-1.0d0)*0.04d0
...
!if(ngpHD .eq. 1) transMatHD=1.0d0
...
!   transMatHD(1,1)=0.97d0
!   transMatHD(ngpHD,ngpHD)=0.95d0
!   transMatHD(ngpHD,ngpHD-1)=1.0d0-transMatHD(ngpHD,ngpHD)-0.01d0
!   transMatHD(ngpHD,1)=0.01d0
!   transMatHD(ngpHD-1,1)=0.39d0
!   transMatHD(ngpHD-1,ngpHD)=0.01d0
!   transMatHD(ngpHD-1,ngpHD-1)=0.60d0
!   transMatHD(1,ngpHD-1)=1.0d0-transMatHD(1,1)-0.01d0
!   transMatHD(1,ngpHD)=0.01d0
!   GridDemand(ngpHD)=nu
!   GridDemand(ngpHD-1)=nu
!   GridDemand(1)=nu/7.5d0

   if(MarkovSwitch .eq. 0) then

      transMatHD(1,1)=0.95d0 !0.95d0
      transMatHD(ngpHD,ngpHD)=0.95d0 !0.95d0
      transMatHD(ngpHD,ngpHD-1)=1.0d0-transMatHD(ngpHD,ngpHD)-0.01d0
      transMatHD(ngpHD,1)=0.01d0
      transMatHD(ngpHD-1,1)=0.85d0 !0.85d0
      transMatHD(ngpHD-1,ngpHD)=0.025d0 !0.025d0
      transMatHD(ngpHD-1,ngpHD-1)=0.125d0 !0.125d0
      transMatHD(1,ngpHD)=0.01d0 !0.01d0
...
!      transMatHD(1,1)=(1.0d0-HDX)
!      transMatHD(1,ngpHD-1)=HDX*(1.0d0-HDY)
!      transMatHD(1,ngpHD)=HDX*HDY
!
!      transMatHD(ngpHD,ngpHD)=(1.0d0-HDX)*(1.0d0-HDY)
!      transMatHD(ngpHD,ngpHD-1)=(1.0d0-HDX)*HDY
!      transMatHD(ngpHD,1)=HDX
!      transMatHD(ngpHD-1,1)=HDZ
!      transMatHD(ngpHD-1,ngpHD)=(1.0d0-HDZ)*HDY
!      transMatHD(ngpHD-1,ngpHD-1)=(1.0d0-HDZ)*(1.0d0-HDY)
...
!      GridBelief(1)=nu-0.1d0
!      if(NoRent .eq. 0 .and. ShortTerm .eq. 1) then
      GridBelief(1) = nu -0.08d0 !0.08d0 *.9d0
!      else
!         GridBelief(1) = nu - 0.04d0
!      endif
!      GridBelief(1) = nu-0.055d0 !0.08d0 *.9d0
...
!      GridBelief(1)=zbar*(0.75d0**((1.0d0-alpha_h)/(0.6)))
      GridBelief(1)=zbar*0.70d0 !*0.675d0 !(0.75d0**((1.0d0-alpha_h)/(0.6)))
...
!   GridDemand(1)=nu-0.07d0
   !Small shock
!   GridDemand(1)=nu-0.005d0
!-.10d
...
!                LTVBase = 1.0d0
                ...
                if(doRf .eq. 1) then
                   ...
                   WedgeGrid(iAgg)=1.0d0 !+0.95d0*dble(iAy-ngpAy)
                   !HELOC Val, if increase, LOC20
                   HELOCGrid(iAgg)=0.2d0 !+0.1d0*dble(ngpAy-iAy)
                   RfGrid(iAgg) = rf_base+0.0202d0*dble(iC-ngpC)

                   RmGrid(iAgg)=RfGrid(iAgg)*1.33d0 !+0.0025d0*dble(iAy-ngpAy)
                   !AyGrid(iAgg) = 1.0d0
                   !GridAy = 1.0d0
                   RlGrid(iAgg)=RmGrid(iAgg) !+0.0201d0 ! 0.0404d0+Markupgrid !*dble(iC-1)
                   FCGrid = mortfc
                else
                   ...
                   if(ShortTerm .eq. 0) then
                      ...
                      HELOCGrid(iAgg)=0.2d0+0.1d0*dble(ngpC-iC)
                      Markupgrid(iAgg)=0.01d0+0.004d0*dble(iC-ngpC)
                      RmGrid(iAgg)=RfGrid(iAgg)*1.33d0 !+0.0025d0*dble(iC-ngpC) !+0.0404d0-0.0303d0*dble(ngpAy-iAy)
                      RlGrid(iAgg)=RmGrid(iAgg) !+0.0025d0*dble(iC-ngpC) !+0.0201d0 ! 0.0404d0+Markupgrid !*dble(iC-1)
                   else
                      LTIGrid(iAgg)=100000.0d0
                      HELOCGrid(iAgg)=0.01d0
                      Markupgrid(iAgg)=0.05d0 + 0.02d0*dble(iC-ngpC) !0.01d0+0.004d0*dble(iC-ngpC)
                      RmGrid(iAgg)=RfGrid(iAgg) !+0.0025d0*dble(iC-ngpC) !+0.0404d0-0.0303d0*dble(ngpAy-iAy)
                      RlGrid(iAgg)=RmGrid(iAgg)+Markupgrid(iAgg) !+0.0201d0 ! 0.0404d0+Markupgrid !*dble(iC-1)
                   endif
                ...
                InterestCap = 0.975d0
                DoInterestCap(iAgg) = 0 !1-(ngpC-iC)

                RationInc(iAgg) = 0 !iC-ngpC+1
...
!                RlGrid(iAgg)=RfGrid(iAgg)
                !February Calibration
!                RlGrid(iAgg)=RmGrid(iAgg)+0.0404d0+Markupgrid(iAgg) !*dble(iC-1)
...
print*,DoInterestCap
   ...
   PhgridTemp(ngpPh)  = 0.35d0 !(real(ngpPh)-1.0d0)*0.04d0
   PhgridTemp(1)  = -PhgridTemp(ngpPh)
   lwidth = (PhgridTemp(ngpPh) - PhgridTemp(1)) / real(ngpPh - 1)
