MODULE SolveFunc
  	USE UTILITIES, ONLY: DP, I4B,LGT
	IMPLICIT NONE
	CONTAINS

	SUBROUTINE MY_AMOEBA(p,y,ndim,ftol,func,ITMAX,nwrite,iter)
		USE UTILITIES, ONLY : imaxloc,iminloc,nrerror_continue,swap
		IMPLICIT NONE
		INTERFACE
			FUNCTION func(x)
		...
		INTEGER, INTENT(IN) :: ITMAX,nwrite
	...
	!BL
		SUBROUTINE amoeba_private
		   ...
		   !     if(mod(iter,10)==0) write(*,157) iter, y(ihi), y(ilo)
			...
			if (mod(iter,20)==0 .and. nwrite==1) then
	...
	subroutine MY_DFPMIN(p, gtol,iter,fret,func, nwrite,nwait,itmax)
...
!========================================================================
    ...
    FUNCTION BRENTMIN(AX,BX,CX,FUNC,TOL,XMIN)
			USE UTILITIES, only: nrerror, SHFT
			IMPLICIT NONE
			...
			REAL(8), PARAMETER :: CGOLD=0.3819660_DP,ZEPS=1.0E-3_DP*EPSILON(AX)
			INTEGER(I4B) :: ITER
    ...
    FUNCTION ZBRENTSOLVE(func,x1,x2,tol,fval)
    USE UTILITIES, only:nrerror
    IMPLICIT NONE
    ...
    if ((fa > 0.0d0 .and. fb > 0.0d0) .or. (fa < 0.0d0 .and. fb < 0.0d0)) &
      call nrerror('root must be bracketed for ZBRENTSOLVE')
    c=b
    fc=fb
    do iter=1,ITMAX
    ...
    call nrerror('ZBRENTSOLVE: exceeded maximum iterations')
    ZBRENTSOLVE=b
    fval=fb
    END FUNCTION ZBRENTSOLVE

END MODULE SolveFunc
...
!      -----------------------------------------------------------------|
      ...
      DIMENSION X(*),XL(*),XU(*),W(*)
      INTEGER, INTENT(IN)  :: nmax, mmax, nptmax
        INTERFACE
            SUBROUTINE DFOVEC(N, MV, X, V_ERR)
                IMPLICIT NONE
                INTEGER, INTENT(IN) :: n,mv
                REAL(8), DIMENSION(n), INTENT(IN)   :: x
                REAL(8), DIMENSION(mv), INTENT(OUT) :: v_err
...
!     Make the call of BOBYQB_H.
      ...
      NDIM,W(ISL),W(ISU),W(IXN),W(IXA),W(ID),W(IVL),W(IW),mv,nmax, mmax, nptmax,dfovec)
   40 RETURN
      END subroutine
...
!
      SUBROUTINE BOBYQB_H (N,NPT,X,XL,XU,RHOBEG,RHOEND,IPRINT,&
		   ...
		   SL,SU,XNEW,XALT,D,VLAG,W,mv,nmax, mmax, nptmax,DFOVEC)
...
!     less than NPT. GOPT will be updated if KOPT is different from KBASE.
      ...
      CALL PRELIM_H (N,NPT,X,XL,XU,RHOBEG,IPRINT,MAXFUN,XBASE,XPT,&
       ...
       mv,HQV,GQV,PQV,FVAL_V,v_err,v_beg,FBEG,nmax, mmax,dfovec)
      XOPTSQ=ZERO
      DO 10 I=1,N
      XOPT(I)=XPT(KOPT,I)
...
!     label 650 or 680 with NTRITS=-1, instead of calculating F at XNEW.
   ...
   60 CALL TRSBOX_H (N,NPT,XPT,XOPT,GOPT,HQ,PQ,SL,SU,DELTA,XNEW,D,&
      W,W(NP),W(NP+N),W(NP+2*N),W(NP+3*N),DSQ,CRVMIN,&
       ...
       model_update,opt_update,nmax, mmax)
...
!     useful safeguard, but is not invoked IN most applications of BOBYQA.
  ...
  190 NFSAV=NF
      ...
      CALL RESCUE_H (N,NPT,XL,XU,IPRINT,MAXFUN,XBASE,XPT,FVAL,&
       ...
       VLAG,W,W(N+NP),W(NDIM+NP),mv,HQV,GQV,PQV,FVAL_V,WV, &
        v_err,v_sumpq,v_vquad,v_diff,v_temp,nmax, mmax, nptmax,dfovec)
      model_update = .true.
...
!   BOBYQA, authored by M. J. D. Powell.
      ...
      SUBROUTINE RESCUE_H (N,NPT,XL,XU,IPRINT,MAXFUN,XBASE,XPT,&
       ...
       KOPT,VLAG,PTSAUX,PTSID,W,mv,HQV,GQV,PQV,FVAL_V,WV, &
       v_err,v_sumpq,v_vquad,v_diff,v_temp,nmax, mmax, nptmax,dfovec)
