...
integer     :: inds(2),indsm(2),indsS(2),Phinds(2),Phinds2(2),indsl(2),Phinds3(2)
double precision    ::  vals(2),valsm(2),valsS(2),Phvals(2),Phvals2(2),valsl(2),Phvals3(2),hyperc(5)
...
iAggpath=ngpAgg
iAggpath(101:105)=1
if(ngpAgg .eq. 12 .and. ngpRf .eq. 1) then
   iAggpath=ngpAgg
   iAggpath(31:60)=6
   iAggpath(81:90)=6
                            ...
                            do bc = 1,nbsim
                                ...
                                if(helpWfore .gt. helpWsell) then
                                   ...
                                   if(GeRent .eq. 0) then
                                      ...
                                      hconsdistop(bc,lc,mc,hc,giExo,gij,giPh)=GridHR(1)*thet(gij)
                                   endif
                                ...
                                select case(polind)
                                    ...
                                    case(1) !Foreclosure optimal
                                       if(GeRent .eq. 0) then
                                          ...
                                          hconsdistop(bc,lc,mc,hc,giExo,gij,giPh)=hconsdistop(bc,lc,mc,hc,giExo,gij,giPh)+(1.0d0-thet(gij))*GridHR(1)
                                       endif
                                    ...
                                    case(3) !Refinance optimal
                                        ...
                                        hconsdistop(bc,lc,mc,hc,giExo,gij,giPh)=hconsdistop(bc,lc,mc,hc,giExo,gij,giPh)+GridH(hc)*(1.0d0-thet(gij))
                                ...
                                select case(polind)
                                    ...
                                    case(1) !Foreclosure optimal
                                       ...
                                       if(GeRent .eq. 0) then
                                          ...
                                          hconsdistop(bc,lc,mc,hc,giExo,gij,giPh)=GridHR(1)
...
do it=tstart,tend
   ...
   if( ((it .eq. 99) .OR. (it .eq. 159) .or. (it .eq. 199) .or. (it .eq. 239) .or. (it .eq. 289) .or. (it .eq. 329) .or. (it .eq. 389) .or. (it .eq. 439) .or. (it .eq. 359) .or. (it .eq. 419)) .AND. (DoIRF .eq. 1)) then
    ...
    if(((it .eq. 166) .or. (it .eq. 206)) .and. (modify .eq. 1) .and. (DoIRF .eq. 1) ) then
        ...
        if(abs(sum(distop-disto)) .gt. 1.d-10) then
            print*,'Something messed up with distribution line 960'
            exit
        else
            disto=distop
            distop=0.0d0
   ...
   if(AdjustCostModel .eq. 0) then
    ...
    if(abs(sum(distr)+sum(disto)-dble(Jtot)) .gt. 0.001d0) print*,'Distribution messed up'
    do giPh=1,ngpPh
    ...
    CALL basefun(PhGrid,ngpPh,exp(log(phh)+log(gelas)),Phvals2,Phinds2)
   ...
   if(AdjustCostModel .eq. 0) then
    ...
    if(Display==0) print*,it,gPh,giAgg,Ht
    ...
    ephtemp=sum(transMatAgg(giAgg,:)*phptemp)
    Pr=sum(Phvals*PrGrid(Phinds,giAgg))
    gPr=Pr
    Prpath(it)=Pr
    Htpath(it)=Ht
    distop=0.0d0
    ...
    if((it .eq. 165)) then
      if(exogenous .eq. 0) then
        ...
        DO bc=1,nbsim
           do gij=1,7
              CALL basefun(PhGrid,ngpPh,exp(log(phh)+Goldminevals(gij)),Phvals3,Phinds3)
...
!
       ...
       if(ij .lt. Jtot) then
          ...
          agepathEown(it,ij) = SUM(disto(:,:,:,:,:,ij))
