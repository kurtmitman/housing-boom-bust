...
subroutine ComputeLOM
    ...
    do giPh=1,ngpPh
        ...
        if(GeRent .eq. 0) then
           PrGrid(giPh,giAgg)=0.053d0*(((1.0d0+dble(BiAnnual)))*(PhGrid(giPh)))**rent_elas
        else
    ...
    IF(NoRentalBelief .eq. 1) THEN
       DO giPh = 1, ngpPh
       DO giAgg = 1, ngpAgg
          IF(AtoD(giAgg) .eq. 2) THEN
             PrGrid(giPh,giAgg) = PrGrid(giPh,CHZYtoA(AtoC(giAgg),ngpHD,AtoZ(giAgg),AtoY(giAgg)))
          ENDIF
       ENDDO
       ENDDO
    ENDIF
